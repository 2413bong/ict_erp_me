...
(
	...
	-- Depart_info 테이블의 부서코드
	dicode char(6) NOT NULL,
	-- Level_info 테이블에 레벨번호
...
';
COMMENT ON COLUMN Level_Info.liLevel IS '실제 레벨';
COMMENT ON COLUMN Level_Info.liDesc IS '레레벨권한에대한 설명';
COMMENT ON COLUMN Member_info.miPwd IS 'SHA256 암호화로직 추가예정
';
COMMENT ON COLUMN Member_info.dicode IS 'Depart_info 테이블의 부서코드';
COMMENT ON COLUMN Member_info.liLevel IS 'Level_info 테이블에 레벨번호';
